...
class UserRepository extends BaseRepository
{
     ...
     * @return mixed
     */
    public function getActivePaginated($paged = 25, $orderBy = 'created_at', $sort = 'desc', $role = []) : LengthAwarePaginator
    {
        return $this->model
    ...
    public function getActive($query = [])
    {
        ...
        ->active();
        if(isset($query['role'])) {
            $users->whereHas('roles', function($q) use ($query) {
    ...
    {
        $users = $this->model
        ->with('roles', 'permissions', 'providers', 'pembimbing', 'bimbingan')
        ->active();
        ...
        if ($id) $users->where('pembimbing_id', $id);
    ...
    {
        $users = $this->model
        ->with('roles', 'permissions', 'providers', 'mahasiswa')
        ->active();
