        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    Mahasiswa <small class="text-muted">Daftar Mahasiswa</small>
                </h4>
            </div><!--col-->
            ...
            <div class="col">
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                          @foreach ($mahasiswa as $k => $b)
                          <tr>
                            <td>
                                <strong>{{$k+1 .'. '.$b->user->name}} </strong>
                                <p class="mb-0">NIM : {{$b->nim}}</p>
                                <p class="mb-0">Tahun : {{$b->tahun}}</p>
                                <p class="mb-0">Alamat : {{$b->alamat }}</p>
                                {{-- <p class="mb-0">Jilid : {{$b->jilid }}</p> --}}
                            </td>
                            <td>{!! $b->actions !!}</td>
                          </tr>
                          @endforeach
...
function deleteItem(url) {
    if (confirm('Yakin Menghapus Mahasiswa?')) {
        // console.log(url);
        $.ajax({
            url: url,
            method: 'POST',
...
function reloadData(kelas) {
    window.location = '/admin/mahasiswa?kelas='+kelas
}
</script>
@endpush
