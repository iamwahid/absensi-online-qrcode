...
@section('content')
{{ html()->modelForm($jadwal, 'POST', route('admin.jadwal.update', $jadwal))->class('form-horizontal')->open() }}
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-5">
                    ...
                    <div class="col-sm-12 col-lg-4">
                        <div class="form-group">
                            ...
                            <select name="matkul_id" id="matkul_id" class="form-control" data-url="{{route('admin.mata_kuliah.index')}}">
                                ...
                                @foreach ($matkul as $m)
                                <option value="{{$m->id.'_'.$m->sks}}" {{$jadwal->matkul == $m ? 'selected' : ''}}>{{$m->nama}}</option>
                                @endforeach
                            </select>
                        </div><!--form-group-->
                    </div>
                    <div class="col-sm-12 col-lg-4">
                        <div class="form-group">
                            ...
                            {{ html()->select('dosen_id')
                                ->options([
                                    '' => 'Pilih'
                                ] + $jadwal->matkul->dosens->pluck('user.name', 'id')->toArray())
                ...
                <div class="row">
                    ...
                    <div class="col-sm-12 col-lg-4">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="start_at" id="start_at" value="{{$jadwal->start_at}}">
                    ...
                    <div class="col-sm-12 col-lg-4">
                        <div class="form-group">
                            ...
                            <input type="hidden" name="finish_at" id="finish_at" value="{{$jadwal->finish_at}}">
                            {{ html()->text('finish_time')
                                ->attribute('type', 'time')
                                ->attribute('readonly', true)
                                ->class('form-control')
                                ->value($jadwal->finish_time)
                                ->required()
                                }}
                        </div><!--form-group-->
                    </div>
